...
name_ver="`echo "$link_ker" | awk -F '/' '{print $NF}' | sed "s/.tar.xz//g"`"
ver="`echo $name_ver | sed "s/^.*linux-\(.*\)\.tar.*$/\1/g"`"
...
cp /boot/config-* /usr/src/linux-$ver/ &&
mv /usr/src/linux-$ver/config-* /usr/src/.config &&
